        ...
        <div className="relative flex h-[140px] w-[180px] flex-col overflow-hidden rounded-md">
          ...
          {place.images[0] && (
            <Image
              ...
              src={getPlaceImageSrc(place.images[0]?.id)}
              alt="widok z miejsca"
              fill
            />
          )}
          ...
          <div className="absolute inset-0 z-20 flex flex-col gap-1 p-1 text-light">
            <div className="mt-1 flex items-center gap-1">
              <div className="relative h-4 w-4">
                <Image
                  src={place.type.icon}
                  alt="znacznik na mapie"
                  layout="fill"
                />
              </div>
              <span className="text-sm ">{place.type.title}</span>
            </div>
            <span className="h-12 text-center text-base line-clamp-2">
              {place.displayName}
            </span>

            <Button
              className="mx-auto mt-auto mb-2 flex justify-between gap-2 "
              variant="filled"
