...
const Button = ({ children, className, isLoading = false, variant = "primary", ...props }: ButtonProps) => {
    return (
        <button
            {...props}
            className={clsx(
                "text-small-regular flex min-h-[50px] w-full items-center justify-center rounded-sm border px-5 py-[10px] uppercase transition-colors duration-200 disabled:opacity-50",
                {
                    ...
                    "border-gray-900 bg-slate-200 font-bold text-gray-900 shadow-sm shadow-slate-700 hover:bg-gray-100": variant === "secondary",
