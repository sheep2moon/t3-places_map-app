...
import { MdOutlineNoPhotography, MdOutlinePhotoSizeSelectActual } from "react-icons/md";
    ...
    return (
        <>
            <div className="relative -mb-4 h-60 w-full">
                {data.images[0] ? (
                    <Image src={getPlaceImageSrc(data?.images[0]?.id)} alt="widok z miejsca" fill className=" object-contain" />
                ) : (
            ...
            <div className="z-10 rounded-t-3xl border-t-2 border-light/20 bg-light pb-8  dark:bg-dark">
                <div className="mt-2 flex flex-col items-start px-4">
                    ...
                    <PlaceTypeAndRating placeType={data.type} placeReviews={data.reviews} />
                </div>
                <div className="mx-1 mt-4 grid grid-cols-2 gap-1">
                    ...
                    <button onClick={handleFlyToPlace} className="flex items-center justify-center gap-2 rounded-sm border-2 border-indigo-900  px-2 py-1 dark:bg-indigo-700">
