...
const PlaceTypeFilter = ({ placeTypes }: PlaceTypeFilterProps) => {
  ...
  const { selectedTypeId, setSelectedTypeId } = usePlacesMapStore(
    (state) => state
  );
  ...
  useEffect(() => {
    ...
    if (typeId) {
      const selectedTypeIndex = placeTypes.findIndex(
        (placeType) => placeType.id === typeId
      );
  ...
  return (
    <>
      <div className={clsx("absolute bottom-2 left-2 z-[999]")}>
        <button
          className={clsx(
            "w-full rounded-full bg-dark p-4 shadow-sm shadow-indigo-600 transition-[width] small:hidden",
            { "w-60 rounded-sm ": isExpanded }
          )}
      ...
      <div
        className={clsx(
          "relative block shrink-0 bg-primary shadow-md shadow-black transition-all hover:w-64 hover:transition-all",
          { "w-64": isExpanded, "hidden w-[72px] small:block": !isExpanded }
        )}
      >
        <div className={clsx("flex h-12 items-center overflow-hidden")}>
          <FiFilter className="ml-5 shrink-0 text-3xl" />
          <p className="ml-6 overflow-hidden whitespace-nowrap font-bold">
            Filtruj typ miejsca
          </p>
        </div>
        <div
          className={clsx(
            "absolute -left-1 flex h-16 w-1 translate-y-12 flex-col justify-center rounded-r-md bg-indigo-600  transition-all",
            { hidden: indicatorPosition === null }
          )}
          style={{
        ...
        <ul className="flex flex-col rounded-t-md text-center text-sm font-medium text-gray-500 dark:text-gray-400">
          {placeTypes?.map((place, index) => (
            ...
            >
              <button
                className={clsx(
                  "flex items-center gap-2 hover:border-indigo-400 hover:text-gray-600 dark:hover:text-gray-300"
                )}
