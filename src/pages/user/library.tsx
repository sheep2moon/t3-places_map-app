...
const LibraryPlace = ({ place }: { place: Place & { type: PlaceType } }) => {
    ...
    return (
        <div className="flex items-center rounded-sm py-1 pr-1 ">
            <div>
                <PlaceTypeIcon size="sm" placeType={place.type} />
            </div>
            <span>{place.displayName}</span>
            <button onClick={handleGoToPlace} className="ml-auto flex items-center gap-1 rounded-sm p-2 hover:bg-primary/5 dark:text-amber-200 dark:hover:bg-light/5">
