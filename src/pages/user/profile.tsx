    ...
    <div className="flex w-full max-w-screen-xl flex-col">
        ...
        <div className="mt-4 text-center sm:mx-auto sm:grid sm:max-w-3xl sm:grid-cols-3 sm:gap-8">
          <div>
            <p className="text-5xl font-extrabold leading-none text-white">
              {data.UserLibrary?.visited.length}
            </p>
      ...
      <Tab.Group as="div" className="mt-4">
        <Tab.List className="flex bg-light/5">
          {tabList.map((tab) => (
            <Tab
              ...
              className={({ selected }) =>
                clsx(
                  "flex w-full items-center justify-center gap-2 rounded-lg py-2.5 text-xl text-sm font-medium leading-5 ",
                  "ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2",
                  {
                    "bg-indigo-400 font-bold text-indigo-900 shadow": selected,
                    "text-light hover:bg-white/[0.12] hover:text-white":
                      !selected,
                  }
                )
              }
        ...
        <Tab.Panels>
          <Tab.Panel>
            <div className="mt-2">
              {data.UserLibrary?.wishlist.length === 0 && (
                <span className="ml-4 text-xs">Brak ulubionych miejsc</span>
              )}
          ...
          <Tab.Panel>
            <div className="mt-2">
              {data.UserLibrary?.visited.length === 0 && (
                <h4 className="mt-8 text-center text-lg">
                  Brak miejsc na liście do odwiedzenia
                </h4>
              )}
          ...
          <Tab.Panel>
            <div className="flex flex-col gap-2 py-2">
              {data.places.map((place) => (
                <div
                  key={place.id}
                  className="flex w-full items-center rounded-sm bg-black/20 py-2 px-2 text-xl"
                >
                  <PlaceTypeIcon placeType={place.type} size="sm" />
                  <div className="ml-4">{place.displayName}</div>
                  <Link
                    href={`/user-places/${place.id}`}
              ...
              {data.places.length === 0 && (
                <div className="flex items-center gap-1 py-1 text-xl">
                  ...
                  <Link
                    href="/add-place"
                    className="rounded-sm bg-amber-200 p-1 text-xl text-primary"
                  >
                    Dodaj miejsce
                  </Link>
                </div>
  ...
  return (
    <div className="flex items-center gap-2 rounded-sm py-1 pr-1 ">
      ...
      <button
        ...
        className="ml-auto flex items-center gap-2 rounded-sm p-2 hover:bg-primary/5 dark:text-amber-200 dark:hover:bg-light/5"
      >
